...
app.use(
  morgan("combined", { stream: { write: (message) => logger.info({ message: message.trim() }) } })
);

app.get("/health", (_req, res) => res.json(ok({ service: "screening-service", status: "up" })));
...
connectDb(env.mongodbUri)
  .then(() => {
    app.listen(env.port, () => logger.info({ message: `Screening service running on port ${env.port}` }));
    startWorker();
  })
  .catch((error) => {
